...
def plot_catalogue_position(csv_name):
    """
    ...
    with open(csv_name, 'r', newline='') as file:
        ...
        for row in reader:
            if count == 0:
                ...
                x_width = data_map.shape[1]
                y_width = data_map.shape[0]
            elif count == 1:
                count += 1
            else:
                ax1.plot(x*float(row[5])+float(row[1]),
                    y_width-y*float(row[5])-float(row[2]), zorder=3, color="C3")
