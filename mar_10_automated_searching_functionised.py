...
def expand_apature_to_threshold(data_map, galatic_centre, back_mean, back_std,
    ...
    while True:
        ...
        if True:
            ...
            d_total_flux = total_flux[-2] - total_flux[-1]
            d_total_area = total_area[-2] - total_area[-1] #single pixel area=1
            gradient = np.append(gradient, d_total_flux / d_total_area)

            #if growth rate is increasing
            if radius > 2 and gradient[-1] > gradient[-2]:
    ...
    return return_line + (total_area_mids,)


def auto_search(txt_name, tlp, brp, border, loc_back_radius, max_radius,
    ...
    if final_plot:
        plot_auto_search_result(txt_name)
...
if __name__ == "__main__":
    ...
    auto_search("mar_10_data.txt", tlp, brp, **search_params, final_plot = True)

    plot_catalogue("mar_10_data.txt")
